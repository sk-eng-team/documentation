...
```bash cURL
curl -G https://api.external.hirednow.ai/campaigns/results \
...
const response = await fetch(
  `https://api.external.hirednow.ai/campaigns/results?${params}`,
  {
    headers: {
...
response = requests.get(
    'https://api.external.hirednow.ai/campaigns/results',
...
- Increase the `limit` parameter when you need more results per page (useful with pagination)
